...
```plain
┌──────────────────────────────────────────────────────────┐
│ L1 接入层  [已实现: 骨架] [待开发: 完整 CLI / Web API]      │
│   cmd/audit (Cobra CLI) │ cmd/server (Gin + asynq)       │
├──────────────────────────────────────────────────────────┤
│ L2 编排层  [待开发]                                        │
│   asynq 任务队列 → 审计流水线编排 → 结果持久化              │
├──────────────────────────────────────────────────────────┤
│ L3 审计引擎                                               │
│   ├─ internal/analyzer 静态分析器          [待开发]        │
│   ├─ internal/llm      LLM 分析器          [待开发]        │
│   └─ internal/sandbox  沙箱执行器          [阶段二]        │
├──────────────────────────────────────────────────────────┤
│ L4 数据层                                                 │
│   ├─ rules/rules.yaml  规则库              [待开发: 40 条]  │
│   ├─ 白名单机制                           [待开发]          │
│   ├─ PostgreSQL + S3 样本/报告/徽章         [待开发]          │
└──────────────────────────────────────────────────────────┘
```
